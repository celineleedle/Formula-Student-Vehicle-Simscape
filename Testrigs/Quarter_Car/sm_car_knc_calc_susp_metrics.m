...
%       veh_sW2W         Hardpoint for front left corner of chassis [x y z] (m)
...
if(toeRZ0ToeL20 == 0)
    ...
    % Standard Turning Circle Diameter, formula to centerline
    maxToe = (abs(toeZ0Avg(end))+abs(toeRZ0Avg(end)))/2;
    TCDCtr = 2*veh_wheelbase/(sqrt(2-2*cosd(maxToe)));

    % Turning Circle Diameter, Curb-to-Curb
    TCDK2K = 2*(veh_wheelbase/sind(abs(toeRZ0Avg(end)))+tireW);

    % Turning Circle Diameter, Wall to Wall
    % Obtain legs of triangle using offset (Wall to Wall)-(wheel center) 
    LongOffsetW2W = veh_sW2W(1);
    LatOffsetW2W  = veh_sW2W(2)-veh_track/2;

    LongW2W  = veh_wheelbase+LongOffsetW2W;
    LatW2W   = veh_wheelbase/tand(toeRZ0Avg(end))+LatOffsetW2W;
    TCDW2W   = sqrt(LongW2W^2+LatW2W^2)*2;
end
...
Names = [...
    ...
    "TCD Kerb to Kerb","TCD Wall to Wall",...
    "Max Rack Force",...
...
Description = [...
    ...
    "To Outer Front Wheel","To Outer Front Body Corner",...
